...
Register-ArgumentCompleter -Native -CommandName 'fs' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'fs' {
            ...
            [CompletionResult]::new('--sep', '--sep', [CompletionResultType]::ParameterName, 'Separator printed after each result')
            [CompletionResult]::new('--format', '--format', [CompletionResultType]::ParameterName, 'Output template for printed results (same syntax as the old `FS_OPTS output_template`)')
            [CompletionResult]::new('--opener', '--opener', [CompletionResultType]::ParameterName, 'Program used to open files on accept (replaces `FS_OPTS opener`)')
            ...
            [CompletionResult]::new('--path-transforms', '--path-transforms', [CompletionResultType]::ParameterName, 'Lua function remapping each result path before rendering/filtering. Values starting with `@` are read from that file')
        ...
        'fs;:custom' {
            ...
            [CompletionResult]::new('-f', '-f', [CompletionResultType]::ParameterName, 'show only files')
            [CompletionResult]::new('--sort', '--sort', [CompletionResultType]::ParameterName, 'sort')
            [CompletionResult]::new('--lua-renderer', '--lua-renderer', [CompletionResultType]::ParameterName, 'Lua function rendering the item''s path into its display tail (column 2). Values starting with `@` are read from that file')
            [CompletionResult]::new('--lua-render-tail', '--lua-render-tail', [CompletionResultType]::ParameterName, 'Lua function rendering the item''s tail column. Values starting with `@` are read from that file')
            [CompletionResult]::new('--path-transforms', '--path-transforms', [CompletionResultType]::ParameterName, 'Lua function remapping each path before rendering/filtering. Values starting with `@` are read from that file')
            [CompletionResult]::new('--delim', '--delim', [CompletionResultType]::ParameterName, 'Field separator within a record: text after the first occurrence is the tail')
            [CompletionResult]::new('--record-sep', '--record-sep', [CompletionResultType]::ParameterName, 'Split the stream into records on this char instead of newlines')
            [CompletionResult]::new('--verbosity', '--verbosity', [CompletionResultType]::ParameterName, 'verbosity')
            [CompletionResult]::new('--override', '--override', [CompletionResultType]::ParameterName, 'config override')
            [CompletionResult]::new('--config', '--config', [CompletionResultType]::ParameterName, 'config path')
            [CompletionResult]::new('--mm-config', '--mm-config', [CompletionResultType]::ParameterName, 'matchmaker config path')
